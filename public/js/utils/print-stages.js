...
(function() {
  ...
  const STAGES = {
    0:  { key: 'printing',             label: 'Printing',                      icon: I.print,    color: 'var(--accent-green)' },
    1:  { key: 'bed_leveling',         label: 'Auto bed leveling',             icon: I.level,    color: 'var(--accent-blue)' },
    2:  { key: 'heatbed_preheating',   label: 'Heatbed preheating',            icon: I.heat,     color: 'var(--accent-orange)' },
    3:  { key: 'sweeping_xy',          label: 'Sweeping XY mech mode',         icon: I.arrows,   color: 'var(--text-muted)' },
    4:  { key: 'changing_filament',    label: 'Changing filament',             icon: I.refresh,  color: 'var(--accent-orange)' },
    5:  { key: 'm400_pause',           label: 'M400 pause',                    icon: I.pause,    color: 'var(--accent-orange)' },
    6:  { key: 'filament_runout',      label: 'Paused: filament runout',       icon: I.warn,     color: 'var(--accent-red)' },
    7:  { key: 'heating_hotend',       label: 'Heating hotend',                icon: I.heat,     color: 'var(--accent-orange)' },
    8:  { key: 'calibrating_extrusion',label: 'Calibrating extrusion',         icon: I.target,   color: 'var(--accent-blue)' },
    9:  { key: 'scanning_bed',         label: 'Scanning bed surface',          icon: I.scan,     color: 'var(--accent-blue)' },
    10: { key: 'inspecting_first_layer',label: 'Inspecting first layer',        icon: I.search,   color: 'var(--accent-blue)' },
    11: { key: 'identifying_plate',    label: 'Identifying build plate type',  icon: I.box,      color: 'var(--accent-blue)' },
    12: { key: 'calibrating_lidar',    label: 'Calibrating Micro Lidar',       icon: I.scan,     color: 'var(--accent-blue)' },
    13: { key: 'homing',              label: 'Homing toolhead',               icon: I.home,     color: 'var(--accent-blue)' },
    14: { key: 'cleaning_nozzle',     label: 'Cleaning nozzle tip',           icon: I.brush,    color: 'var(--accent-blue)' },
    15: { key: 'checking_extruder',   label: 'Checking extruder temperature', icon: I.thermo,   color: 'var(--accent-orange)' },
    16: { key: 'paused_by_user',      label: 'Paused by user',                icon: I.pause,    color: 'var(--accent-orange)' },
    17: { key: 'front_cover_pause',   label: 'Paused: front cover falling',   icon: I.door,     color: 'var(--accent-red)' },
    18: { key: 'calibrating_lidar_2', label: 'Calibrating Micro Lidar',       icon: I.scan,     color: 'var(--accent-blue)' },
    19: { key: 'calibrating_flow',    label: 'Calibrating extrusion flow',    icon: I.drop,     color: 'var(--accent-blue)' },
    20: { key: 'nozzle_temp_error',   label: 'Paused: nozzle temp malfunction',icon:I.thermo,   color: 'var(--accent-red)' },
    21: { key: 'bed_temp_error',      label: 'Paused: bed temp malfunction',  icon: I.thermo,   color: 'var(--accent-red)' },
    22: { key: 'filament_unloading',  label: 'Filament unloading',            icon: I.up,       color: 'var(--accent-orange)' },
    23: { key: 'skip_step_pause',     label: 'Skip step pause',               icon: I.skip,     color: 'var(--accent-orange)' },
    24: { key: 'filament_loading',    label: 'Filament loading',              icon: I.down,     color: 'var(--accent-orange)' },
    25: { key: 'motor_noise_cal',     label: 'Motor noise calibration',       icon: I.volume,   color: 'var(--accent-blue)' },
    26: { key: 'ams_lost_pause',      label: 'Paused: AMS lost',              icon: I.warn,     color: 'var(--accent-red)' },
    27: { key: 'heatbreak_fan_pause', label: 'Paused: heatbreak fan low speed',icon:I.fan,       color: 'var(--accent-red)' },
    28: { key: 'chamber_temp_error',  label: 'Paused: chamber temp error',    icon: I.thermo,   color: 'var(--accent-red)' },
    29: { key: 'cooling_chamber',     label: 'Cooling chamber',               icon: I.snow,     color: 'var(--accent-blue)' },
    30: { key: 'gcode_pause',         label: 'Paused by G-code',              icon: I.pause,    color: 'var(--accent-orange)' },
    31: { key: 'motor_noise_showoff', label: 'Motor noise showoff',           icon: I.volume,   color: 'var(--text-muted)' },
    32: { key: 'nozzle_filament_covered',label: 'Paused: nozzle filament covered',icon:I.warn,     color: 'var(--accent-red)' },
    33: { key: 'cutter_error',        label: 'Paused: cutter error',          icon: I.scissors, color: 'var(--accent-red)' },
    34: { key: 'first_layer_error',   label: 'Paused: first layer error',     icon: I.alert,    color: 'var(--accent-red)' },
    35: { key: 'nozzle_clog',         label: 'Paused: nozzle clog',           icon: I.alert,    color: 'var(--accent-red)' },
  };
