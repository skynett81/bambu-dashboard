    ...
    try {
      ...
      for (const f of files.slice(0, 50)) {
        ...
        html += `<div style="display:flex;align-items:center;gap:6px;padding:4px 6px;border-radius:4px;font-size:0.72rem;background:var(--bg-inset)">
          ...
          <span class="text-muted" style="font-size:0.6rem;flex-shrink:0">${size}</span>
          <button class="form-btn form-btn-sm" style="font-size:0.55rem;padding:1px 4px" data-ripple onclick="sendCommand('print_file',{filename:'${name}'})">Print</button>
          <button class="form-btn form-btn-sm form-btn-danger" style="font-size:0.55rem;padding:1px 4px" data-ripple onclick="if(confirm('Delete ${name}?')) fetch('/api/printers/${pid}/files/${encodeURIComponent(name)}',{method:'DELETE'}).then(()=>window._refreshPrinterFiles())">×</button>
        </div>`;
      }
      html += '</div>';
